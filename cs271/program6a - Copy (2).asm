TITLE Lower Level Programming (program6a.asm)
...
; Description: Program circumvents the use of irvines library to get and print numeric values
;  by reading a value in as a string and then converting the string to a numeric value through
;  low level programming. First the user is asked to provide 10 unsigned integers. If the integers
;  are validated, they are converted from a string to a numeric number. Each validated numeric 
;  number is then stored in an array. After the array contains all 10 validated numbers, the 
;  program sums the numbers and prints out the sum. To print the sum, the program converts the
;  number back into a string and then prints the sum as a string. Next, the average of all the
;  numbers is calculated, the average is converted to a string, and the string is printed to the
;  screen. The program then prints a farewell message to the user.
...
array		DWORD	10 DUP(0)			;holds the 10 unsigned integers input by user
numVal		DWORD	0					;stores the numeric version of a current working number
strVal		BYTE	15 DUP(0)			;stores the string version of a current working number	
sumVal		DWORD	0					;stores the sum of all array elements
average		DWORD	0					;stores the average of all the array elements
...
redo:
	...
	cmp		eax, 10						;only allows 10 digit numbers
	jg		err
...
calc:
	...
	jc		err							;error if number entered is too big
	mov		edx, [ebp-4]				;move current digit to edx
	sub		edx, 48						;convert character digit to numeric digit
	add		eax, edx					;add current numeric digit to working number
	jc		err							;error if adding the number sets carry flag
...
err:
	...
	mov		edx, 0
	mov		[edi], edx					;clears invalid number from numVal
	jmp		redo

done:
	mov		ecx, 10						;will fill all 10 bytes of strVal with null
...
displayList PROC
	...
	mov		esi, [ebp+40]				;set offset of array to source
	mov		ebx, [ebp+52]				;set offset of numVal
	call	CrLf
