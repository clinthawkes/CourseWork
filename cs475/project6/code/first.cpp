...
main( int argc, char *argv[ ] )
{
    ...
    double check = ((double)NUM_ELEMENTS * (NUM_ELEMENTS - 1)) / 2.; 
	...
	/* { */
		...
		/* { */
            /* /1* //this is for the multadd *1/ */ 
		/* 	/1* fprintf( stderr, "%4d: %13.6f * %13.6f + %13.6f wrongly produced %13.6f instead of %13.6f (%13.8f)\n", *1/ */
		/* 		/1* i, hA[i], hB[i], hC[i], hD[i], expected, fabs(hD[i]-expected) ); *1/ */
		/* 	/1* fprintf( stderr, "%4d:    0x%08x *    0x%08x +  0x%08x wrongly produced    0x%08x instead of    0x%08x\n", *1/ */
		/* 		/1* i, LookAtTheBits(hA[i]), LookAtTheBits(hB[i]), LookAtTheBits(hC[i]), LookAtTheBits(hD[i]), LookAtTheBits(expected) ); *1/ */
    ...
    if(fabs(sum - check) > TOL){
        fprintf(stderr, "Reduction was wrong. Produced %13.6f instead of %13.6f.\n", sum, check); 
    }     
